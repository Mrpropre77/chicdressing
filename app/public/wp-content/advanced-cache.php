...
$possible_plugin_locations = array(
	...
	'/Users/nassimekherbouche/Local Sites/chic-dressing/app/public/wp-content/plugins/wp-optimize/cache',
);
...
if (false !== $plugin_location) {
	...
	if (!defined('WPO_CACHE_EXT_DIR')) define('WPO_CACHE_EXT_DIR', '/Users/nassimekherbouche/Local Sites/chic-dressing/app/public/wp-content/plugins/wp-optimize/cache/extensions');
}

if (!@file_exists(WPO_CACHE_CONFIG_DIR . '/config-chic-dressing.local.php')) { return; }

$GLOBALS['wpo_cache_config'] = @json_decode(file_get_contents(WPO_CACHE_CONFIG_DIR . '/config-chic-dressing.local.php'), true);

if (empty($GLOBALS['wpo_cache_config'])) {
	include_once(WPO_CACHE_CONFIG_DIR . '/config-chic-dressing.local.php');
}
